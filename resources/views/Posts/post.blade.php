...
@section('page')
    ...
    <div class="
    w-11/12
    container mx-auto content-center m-10 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 gap-5">

        @if($posts->count() > 0)
            @foreach ($posts as $post)
                <div class="
    h-full
    max-w-screen-sm rounded overflow-hidden shadow-lg ">
                    <img class="w-full"
